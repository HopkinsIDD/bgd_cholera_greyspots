...
fig_nums(name = "fig-buffers", caption = "**Map of 10 km, 30 km, and 50 km buffers around sentinel hospital sites (left, center, right, respectively).**", display = FALSE)
...
## Mapping cholera infections with serological data
...
Using data from a nationally-representative survey across Bangladesh, we developed maps of cholera infection across 5 km x 5km cells in Bangladesh. This map suggests that `r tot_inf` cholera infections occurred in Bangladesh over the past year (estimated population size ###) [NEED TO ADD DETAILS ON TIME PERIOD FOR THESE ESTIMATES]. We standardized the infection rate estimates in each cell by the population-weighted mean of the Bangladesh infection rate in that cell. This yielded a relative risk of cholera infection for each grid cell (`r fig_nums("fig-sero-rr-map", display = "cite")`). 
...
fig_nums(name = paste0("fig-sero-rr-map"), caption = paste("**Risk of cholera infection relative to population-weighted mean**"), display = "false")
...
We examined the distribution of relative risks (`r fig_nums("fig-sero-rr-hist", display = "cite")`) and estimated cholera infections (`r fig_nums("fig-sero-inf-hist", display = "cite")`) by grid cell in order to identify thresholds for binning cholera infection risk into categories .

```{r, fig-sero-rr-hist, fig.dim = c(3,3)}

hist(sero$rr2_cor, main = "", xlab = "Relative risk of cholera infection")
fig_nums(name = paste0("fig-sero-rr-hist"), caption = paste("**Histogram of the risk of cholera infection relative to population-weighted mean**"), display = "false")
...
hist(log(sero$inf), main = "", xlab = "Log total cholera infections")
fig_nums(name = paste0("fig-sero-inf-hist"), caption = paste("**Histogram of the log total number of cholera infections**"), display = "false")

```

`r fig_nums("fig-sero-inf-hist", display = "full")`

We used two sets of fixed thresholds to delineate areas with high, moderate, and low cholera infection risk. The first set of thresholds was based on low, moderate, and high risk (RR) of cholera infection relative to the mean cholera infection rate across Bangladesh adjusted by the population of each cell (i.e. the expected infection rate in the cell). 5 km x 5km grid cells were categorized as low risk if RR < 0.5, moderate risk if RR >= 0.5 and RR < 1.5, and high risk if RR >= 1.5 (`r fig_nums("fig-sero-rrcat", display = "cite")`). The second set of threshold was based on absolute case counts, where grid cells were categorized as low risk if infections < 2000, moderate risk if infections >= 2000 and infections < 5000, and high risk if infections >= 5000 (`r fig_nums("fig-sero-infcat", display = "cite")`).
...
fig_nums(name = paste0("fig-sero-rrcat"), caption = paste("**Cholera risk map as categorized by the risk of cholera infection relative to a population-weighted mean**"), display = "false")
...
## Intersecting cholera infections and the cholera surveillance zone

We overlaid the cholera surveillance zone with the binned cholera infection risk maps to examine the distribution of risks in the surveilled areas. This was done for infection risk as categorized by relative risk (`r fig_nums(paste0("sero-buffers-rr-", buffer_labels[1], "-plt"), display = "cite")`-`r fig_nums(paste0("sero-buffers-rr-", length(buffer_labels), "-plt"), display = "cite")`) and by total infections (`r fig_nums(paste0("sero-buffers-inf-", buffer_labels[1], "-plt"), display = "cite")`-`r fig_nums(paste0("sero-buffers-inf-", length(buffer_labels), "-plt"), display = "cite")`).
...
for(i in 1:length(buffer_labels)){
  ...
  cat(fig_nums(name = paste0("sero-buffers-rr-", km_label, "-plt"), caption = paste0("**Risk of cholera infection, as categorized by relative risk, within cholera surveillance zones (**", km_label, "**)**"), display = "full"))
}
...
for(i in 1:length(buffer_labels)){
  ...
  cat(fig_nums(name = paste0("sero-buffers-inf-", km_label, "-plt"), caption = paste0("**Risk of cholera infection, as categorized by number of infections, within cholera surveillance zones (**", km_label, "**)**"), display = "full"))
}
