...
km_vec_ls <- list(B1= c(10, 20, 30), B2 = c(10, 10, 30), B3 = c(20, 20, 30), B4 = c(15, 25, 35)) 
...
mp_filenames <- grep("multipoly_mixed", list.files("generated_data/", pattern = "*.shp"), value = TRUE)
...
sp_filenames <- grep("singlepoly_mixed", list.files("generated_data/", pattern = "*.shp"), value = TRUE)
...
display_rrcat_in_buffers <- catch_df %>% 
  ...
  dplyr::select(buff, rr_risk_cat, num_inf, perc_inf, perc_pop)

knitr::kable(display_rrcat_in_buffers, format = "pandoc", booktabs = T, align = "c", row.names = FALSE, col.names = c("Buffer size", "Risk Category", "Number Infected", "Infections in Surveillance Zone (%)", "Population in Surveillance Zone (%)"))
