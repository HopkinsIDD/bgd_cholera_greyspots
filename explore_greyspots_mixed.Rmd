...
knit: (function(inputFile, encoding) { 
          rmarkdown::render(inputFile,
                        encoding=encoding, 
                output_file='explore_greyspots_mixed_20200112.html') })
---
...
r <- raster(m, res = .05)
pop_rast <- fasterize(m, r, field = "pop", fun="last")
pop_rast_v<- velox(pop_rast)

out_wd <- "generated_data/mixed/"
fig_wd <- "figures/"
...
for(i in 1:length(km_vec_ls)){
  ...
  hosp_dat2 <- hosp_dat %>%  
    dplyr::mutate(buff_radius = ifelse(Type == "subdistrict", km_vec[1], ifelse(Type == "district", km_vec[2], ifelse(Type == "tertiary", km_vec[3], NA))))
