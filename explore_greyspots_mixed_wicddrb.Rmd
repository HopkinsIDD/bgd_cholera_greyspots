...
hosp_dat <- read_csv("data/hosp_GPS_with_icddrb.csv")
...
for(i in 1:length(km_vec_ls)){
  ...
  hosp_dat2 <- hosp_dat %>%  
    dplyr::mutate(buff_radius = ifelse(Type == "subdistrict", km_vec[1], ifelse(Type == "district", km_vec[2], ifelse(Type == "tertiary", km_vec[3], ifelse(Type == "special", km_vec[4], NA)))))
...
for(i in 1:length(buffer_labels)){
  ...
  plt <- ggplot() + 
    ...
    scalebar(adm2_shp, dist = 50, dist_unit = "km",transform = TRUE, model = "WGS84", st.size = 2)
  print(plt)
  ggsave(filename = paste0("figures/sero-nonbuffers-rr-", km_label, ".pdf"), plt, width = 6.5, height = 4, units = "in")
...
for(i in 1:length(buffer_labels)){
  ...
  plt <- ggplot() + 
    ...
    scalebar(adm2_shp, dist = 50, dist_unit = "km",transform = TRUE, model = "WGS84", st.size = 2)
  print(plt)
  ggsave(filename = paste0("figures/sero-nonbuffers-inf-", km_label, ".pdf"), plt, width = 6.5, height = 4, units = "in")
