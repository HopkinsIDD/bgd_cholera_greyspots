...
subtitle: "January 12, 2020 - with icddr,b hospital"
author: "Elonia"
...
for(i in 1:length(km_vec_ls)){
  ...
  if(!file.exists(paste0(out_wd, "buff_sf_singlepoly_mixed_", paste(km_vec, collapse = "-"), "km.shp"))){
    st_write(sp_file, paste0(out_wd, "buff_sf_singlepoly_mixed_", paste(km_vec, collapse = "-"), "km.shp"))
  }
  ...
  if(!file.exists(paste0(out_wd, "buff_sf_multipoly_mixed_", paste(km_vec, collapse = "-"), "km.shp"))){
    st_write(mp_file, paste0(out_wd, "buff_sf_multipoly_mixed_", paste(km_vec, collapse = "-"), "km.shp"))
  }
...
mp_filenames <- grep("multipoly_mixed", list.files(out_wd, pattern = "*.shp"), value = TRUE)
buff_mp_ls <- lapply(mp_filenames, function(fn){
  st_read(paste0(out_wd, fn))
})
mp_names <- sub(".shp", "", sub("buff_sf_multipoly_mixed_", "", mp_filenames))
names(buff_mp_ls) <- mp_names

## import buffer files (single polygon versions)
sp_filenames <- grep("singlepoly_mixed", list.files(out_wd, pattern = "*.shp"), value = TRUE)
buff_sp_ls <- lapply(sp_filenames, function(fn){
  st_read(paste0(out_wd, fn))
})
sp_names <- sub(".shp", "", sub("buff_sf_singlepoly_mixed_", "", sp_filenames))
...
There are 23 sentinel hospital sites (`r tab_nums("tab-hospnames", display = "cite")`).
...
print(plt)
ggsave(filename = paste0(fig_wd, "sero-rr.pdf"), plt, width = 6, height = 6, units = "in")
