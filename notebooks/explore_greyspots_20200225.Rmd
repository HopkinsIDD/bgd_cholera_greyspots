...
knit: (function(inputFile, encoding) { 
          rmarkdown::render(inputFile,
                        encoding=encoding, 
                output_file='explore_greyspots_mixed_wicddrb_20200112.html') })
---
...
We define cholera surveillance zones as geographies surrounding one of the 23 cholera sentinel hospitals. Surveillance zones are defined as the area enclosed within a circular buffer with a radius of x km surrounding the coordinates of a given sentinel hospital. Subdistrict, district, and tertiary care hospitals may have different radii.
...
std_crs_txt <- "+proj=tmerc +lat_0=0 +lon_0=90 +k=0.9996 +x_0=500000 +y_0=0 +a=6377276.345 +b=6356075.41314024 +towgs84=283.7,735.9,261.1,0,0,0,0 +units=km +no_defs"
gadm_crs_txt <- "+proj=longlat +datum=WGS84 +no_defs"

## sentinel hospital location data
hosp_dat <- read_csv("data/hosp_GPS_wicddrb.csv")
hosp_dat_gadmcrs <- sf::st_as_sf(hosp_dat %>% dplyr::select(LON,LAT), coords = c("LON","LAT"), crs = gadm_crs_txt) ## lat/lon coords on different crs

## transform hospital coords to match standard bgd crs in km
hosp_dat_stdcrs <- st_transform(hosp_dat_gadmcrs, std_crs_txt)
...
# } else{
  ...
  sero_import[dup_ix,]$pop_toAgg <- 0
  sero_import[dup_ix,]$implied_inf_toAgg <- 0

  ## transform sero data to match standard bgd crs in km
  sero <- st_transform(sero_import, crs = std_crs_txt)

  saveRDS(sero, paste0("data/clean_sero_bgdintersect", code_str, ".rds"))
# }

# ## 1/12/20 ECL: THERE IS A PROBLEM WITH THE RASTERIZATION. WHEN YOU SUM THE POPULATION AFTER RASTERIZATION ITS IN THE BILLIONS, BUT IT'S A REASONABLE 163 MILLION BEFORE THAT STEP
...
for(i in 1:length(km_vec_ls)){
  ...
  sero_buffer_sp <- sf::st_intersection(sero, buff)
  saveRDS(sero_buffer_sp, paste0(out_wd, "sero_buff_sf_singlepoly", code_str, "_", paste(km_vec, collapse = "-"), "km.rds"))


  ## multipolygon for all hospitals
  ## combine buffer polygons per hospital into single multipolygon
  buff_mp <- st_sf(st_union(buff))
  ...
  ## transform mp buffer back to GADM shapefiles crs
  buff_mp_gadmcrs <- st_transform(buff_mp, gadm_crs_txt) 

  ## save plot
  buff_map <- map_buffers(adm0_shp, adm2_shp, hosp_dat_gadmcrs, buff_mp_gadmcrs)
...
There are 23 sentinel hospital sites that perform laboratory confirmation of _V. cholerae_ in Bangladesh (`r tab_nums("tab-hospnames", display = "cite")`).
...
In the absence of better data on health care utilization around the hospital sentinel surveillance sites, we assumed that buffers with radii of `r buffer_labels` around the hospital could serve as proxies for potential hospital catchment areas (`r fig_nums("fig-map-buffers", display = "cite")`). Subdistrict, district, tertiary care sentinel hospitals and the icddr,b Dhaka cholera hospital could have buffer radii of different sizes. We refer to the joint buffer areas as the "cholera surveillance zone" for the national cholera sentinel surveillance system.

```{r, fig-buffers}

buffermaps <- lapply(buffer_labels, function(dist){
  map_buffers(adm0_shp, adm2_shp, hosp_dat_gadmcrs, buff_mp_ls[[dist]])
}) 

wrap_plots(buffermaps) 
fig_nums(name = "fig-buffers", caption = paste("**Map of**", paste(buffer_labels, collapse = ", "), "**buffers around (subdistrict-district-tertiary-icddr,b) sentinel hospital sites (left to right, top row to bottom row, respectively).**"), display = FALSE)
...
tot_inf <- sum(sero$implied_inf_toAgg)
...
sero_gadmcrs <- st_transform(sero, gadm_crs_txt)

plt <- ggplot() + 
  geom_sf(data = adm0_shp, fill=NA, lwd=0.1, alpha=.1) + 
  geom_sf(data = sero_gadmcrs, aes(fill=rr_med_bound), color=NA, lwd=0) +
  geom_sf(data = hosp_dat_gadmcrs, shape=3, color="black", size=1, lty=4) +
...
hist(sero$rr_med_bound, main = "", xlab = "Relative risk of infection")
...
hist(log(sero$implied_inf_toAgg), main = "", xlab = "Log total infections")
...
print("Distribution of median relative risk across by 5 km x 5 km cell")
cat(summary(sero$rr_median))

print("Distribution of median number of infections by 5 km x 5 km cell")
cat(summary(sero$implied_inf_toAgg))
...
sero_gadmcrs <- sero_gadmcrs %>% 
  dplyr::mutate(rr_risk_cat = ifelse(rr_median < 0.5, "Low", ifelse(rr_median >= 0.5 & rr_median < 1.5, "Moderate", "High")),
                inf_risk_cat = ifelse(implied_inf_toAgg < 2000, "Low", ifelse(implied_inf_toAgg >= 2000 & inf < 5000, "Moderate", "High"))) %>% 
...
sero_stdcrs <- st_transform(sero_gadmcrs, std_crs_txt)
...
plt <- ggplot() + 
  geom_sf(data = adm0_shp, fill = NA, lwd = 0.1, alpha = .1) + 
  geom_sf(data = sero_gadmcrs, aes(fill = rr_risk_cat), color = NA, lwd = 0) +
  # geom_sf(data = mp_file, lwd = 0.3, alpha = 0.1) + ## these buffers shouldn't be here
  geom_sf(data = hosp_dat_gadmcrs, shape = 3, color = "black", size = 1, lty = 4) +
  scale_colour_manual(values = c("#e41a1c","#377eb8","#4daf4a","#984ea3")) +
  # scale_fill_gradient2(low ="#4191e1", mid = "white", high = "#b20000", midpoint=1.0) +
  coord_sf(datum = NA) + 
...
plt <- ggplot() + 
  geom_sf(data = adm0_shp, fill = NA, lwd = 0.1, alpha = .1) + 
  geom_sf(data = sero_gadmcrs, aes(fill = inf_risk_cat), color = NA, lwd = 0) +
  # geom_sf(data = mp_file, lwd = 0.3, alpha = 0.1) + ## these buffers shouldn't be here
  geom_sf(data = hosp_dat_gadmcrs, shape = 3, color = "black", size = 1, lty = 4) +
  scale_colour_manual(values = c("#e41a1c","#377eb8","#4daf4a","#984ea3")) +
  # scale_fill_gradient2(low ="#4191e1", mid = "white", high = "#b20000", midpoint=1.0) +
...
print(plt)
ggsave(filename = paste0(fig_wd, "sero-infcat", sero_gadmcrs, ".pdf"), plt, width = 6, height = 6, units = "in")
...
sero_buff_stdcrs_ls <- lapply(buffer_labels, function(buff_lab){
  st_intersection(sero_stdcrs, buff_mp_ls[[buff_lab]]) 
})
names(sero_buff_stdcrs_ls) <- buffer_labels


## dataframe version of sero-buffers intersections without geometries
sero_buff_df <- sf::st_as_sf(data.table::rbindlist(sero_buff_stdcrs_ls, idcol = "buff")) %>%
  sf::st_drop_geometry() %>%
  tbl_df %>%
  rowwise %>%
...
for(i in 1:length(buffer_labels)){
  ...
  intersect_sero <- sero_buff_stdcrs_ls[[km_label]]
...
for(i in 1:length(buffer_labels)){
  ...
  cat(fig_nums(name = paste0("sero-nonbuffers-rr-", km_label, "-plt"), caption = paste0("**Infection risk, as categorized by relative risk, outside of cholera surveillance zones (", km_label, " for subdistrict, district, tertiary care, and icddr,b hospitals)**"), display = "full"))
}
...
for(i in 1:length(buffer_labels)){
  ...
  cat(fig_nums(name = paste0("sero-nonbuffers-inf-", km_label, "-plt"), caption = paste0("**Infection risk, as categorized by number of infections, outside of cholera surveillance zones (", km_label, " for subdistrict, district, tertiary care, and icddr,b hospitals)**"), display = "full"))
}
...
Incorporate the uncertainty results here
