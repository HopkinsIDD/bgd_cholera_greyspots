...
knit: (function(inputFile, encoding) { 
          rmarkdown::render(inputFile,
                        encoding=encoding, 
                output_file='explore_greyspots_20200825.html') })
---
...
##Trying thresholds of 0.8 and 1.2
...
## Supplementary Figure 1. B. Map of entropy based on serological incidence with RR cutoff of 2.
...
sfig1B <- ggplot() + 
  ...
  geom_sf(data = sero.sites, shape=2, aes(color="black"), size=1.25, lty=1) +
  scale_fill_continuous_sequential(palette="Magenta", limits=c(0.2, 0.8), breaks=c(0.3, 0.5, 0.7)) +
  coord_sf(datum=NA) + 
  labs(x="") + labs(y="") + labs(fill="Entropy") +
  scale_colour_manual(name="Serosurvey\nsites",values=c('black'),labels=c('Community')) +
  theme_void() 

ggsave(filename = paste0(fig_wd, "sero-entropy", code_str, ".pdf"), sfig1B, width = 6, height = 6, units = "in")



## Supplementary Figure 1. C. Map of median estimated number of absolute V. cholerae infections in Bangladesh 
...
ggsave(filename = paste0(fig_wd, "sero-inf", code_str, ".pdf"), sfig1C, width = 6, height = 6, units = "in")



plt<- ggpubr::ggarrange(sfig1A,sfig1B,sfig1C,nrow=2,ncol=2,labels = c("A","B","C"))
print(plt)
...
sfig_nums(name = "sfig-supp-one-cap", caption = paste("A. **Median risk of _V. cholerae_ seroincidence relative to a population-weighted mean by 5 km x 5 km grid cell.** These relative risk estimates are bounded such that RRs above and below 2 and -2 were plotted as the values 2 and -2, respectively. The black marks indicate sentinel hospital locations. B. **Map of the uncertainty of the seroincidence estimates measured by entropy by 5 km x 5 km grid cell.** The black marks indicate the serosurvey sites from the cross-sectional survey conducted in 2015. C. **Median number of estimated V. cholerae infections per grid cell in the previous year. The black marks indicate sentinel hospital locations.** These estimates were calculated as the product of the median seroincidence risk and 2015 WorldPop population estimate in each grid cell. The black marks indicate sentinel hospital locations."), display = FALSE)
...
# Figure 2. A. Map of categorized relative seroincidence risk by H-M-L and cholera surveillane zones.
...
## Figure 2. B. Map of categorized absolute number of infections by H-M-L and cholera surveillance zones. We redrew the estimated number of infections risk map after binning grid cells into high, moderate, and low risk categories 
...
display_inf_riskpops <- sero2 %>%
  ...
  summarise(pop1 = sum(pop_toAgg, na.rm = TRUE), inf1 = sum(implied_inf_toAgg, na.rm = TRUE)) %>% 
  data.frame %>%
...
fig_nums(name = "fig-bar-plot-cap", caption = "A. Bar plot of the number of people and number of infections in the Bangladesh population and in the cholera surveillance zone living in high-moderate-low risk areas as defined by relative seroincidence risk and absolute number of infections risk measures. B. Bar plot of the percentage of the population and percentage of infection in Bangladesh and the cholera surveillance zone living in high-moderate-low risk areas using the relative seroincidence risk and absolute number of infections as measures of risk.", display = FALSE)
...
## Supplementary Figure 2. B. surv zones with high certainty risk categories inside 
...
sfig2B <- ggplot() + 
  ...
  geom_sf(data = hosp_dat, shape = 3, aes(color = "black"), size = 1) +
  scale_fill_manual("Risk category", values = c("#377eb8", "#4daf4a")) +
  coord_sf(datum = NA) + 
  labs(x="") + labs(y="") + 
  scale_colour_manual(name="Sentinel sites",values=c('black'),labels=c('Hospital')) +
  theme_void() 
...
sfig_nums(name = "sfig-sero-buffers-rr-cap", caption = "A. The relative seroincidence risk within cholera surveillance zones (for subdistrict, district, and tertiary care, and icddr,b hospitals). B. High-certainty grid cells based on the measure of entropy with the cholera surveillance zones overlayed.", display = FALSE)
