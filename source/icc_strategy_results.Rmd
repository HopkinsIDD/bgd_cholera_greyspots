...
for (i in 1:length(tmpdirs)){

  smalltmpdf <- dplyr::filter(tmpdf, dir == tmpdirs[i])
  cl_grid <- purrr::map_dfr(1:nrow(smalltmpdf), function(j){
    readr::read_csv(smalltmpdf[j,]$fn, col_types = cols(metric = col_character())) %>%
      dplyr::mutate(strategy = stringr::str_remove(stringr::str_remove(smalltmpdf[j,]$dir, "^tmp_"), "_buff10-20-30_seed544400_nsims20"))
  })
  ...
  try({
    ...
    print(paste("Working on", unique_strategies))
    rrdat <- dplyr::filter(cl_iter, strategy == unique_strategies & metric == "rr") 
    ...
    cat(
      tab_model(
      title = paste("relative risk thresholds:", unique_strategies),
      r1,
      r2,
      r3,
      transform = NULL,
  ...
  try({
    ...
    print(paste("Working on", unique_strategies))
    infdat <- dplyr::filter(cl_iter, strategy == unique_strategies & metric == "inf") 

    if(params$testing){
      print("Warning: These are testing results!!!")
      infdat <- dplyr::mutate(infdat, rep = factor(c(rep(1, 40), rep(2, 59))), sim = factor(c(rep(1, 20), rep(2, 9), rep(3, 70))))
    }

    infdat <- dplyr::mutate(infdat, rep = factor(rep), sim = factor(sim))

    m1 <- lmer(inf_survzone ~ (1 | rep) , data = infdat)
    m2 <- lmer(inf_survzone ~ (1 | sim), data = infdat)
    m3 <- lmer(inf_survzone ~ (1 | rep) + (1 | sim) , data = infdat)

    cat(
      tab_model(
      title = paste("absolute risk thresholds:", unique_strategies),
      m1,
      m2,
      m3,
      transform = NULL,
...
The cleandata block has memory limitations
```{r cleandata}
## clean data
...
Skipping this section now because of memory limitations.
